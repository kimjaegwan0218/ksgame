...
void move_arrow_key(char key, int *x1, int *y1, int x_b, int y_b, int step);
...
int main(void)
{
   char *slide="■", key;
   int x=1, y=1;
   int h_slide_length, v_slide_length, step;
   printf("슬라이드바 표시\n\n");
   ...
   do
   {
    ...
    key=getch();
    move_arrow_key(key, &x, &y, h_slide_length, v_slide_length, step);
   }while(key!=27);
   		return 0;
}
